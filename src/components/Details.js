...
export default function Details() {
  ...
  const handleSearchSubmit = async () => {
    const present_url = `https://weather-4ggl.onrender.com/details/${location}/`;
  ...
  const getPresentDetails = async () => {
    const present_url = `https://weather-4ggl.onrender.com/details/London/`;
    const response = await fetch(present_url);
    const data = await response.json();
    console.log(data);
    setDetails(data);

    const past_url = `https://weather-4ggl.onrender.com/pastdetails/London/${previousDate}/`;
    const past_response = await fetch(past_url);
    const past_data = await past_response.json();
    console.log(past_data);
    setYesterdayDetails(past_data);

    const future_url = `https://weather-4ggl.onrender.com/futuredetails/London/${nextDate}/`;
    const future_response = await fetch(future_url);
    const future_data = await future_response.json();
    console.log(future_data);
